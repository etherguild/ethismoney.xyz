...
// flex-grow: 1;
      ...
      Object.keys(baseStyles).forEach(category => {
        ...
        if(lineHeightVariants[category])
          Object.entries(lineHeightVariants[category]).forEach(([size, lineHeight]) => {
            newUtilities[`.${category}-${size}`].lineHeight = lineHeight;
          });

        // Font weight variants
        if(fontWeights[category])
          Object.entries(fontWeights[category]).forEach(([weight, fontWeight]) => {
            console.log("category", category, "weight", weight, "fontWeight", fontWeight);
            newUtilities[`.${category}-${weight}`].fontWeight = fontWeight;
          });
          
        // Letter spacing variants
        if(letterSpacingVariants[category])
          Object.entries(letterSpacingVariants[category]).forEach(([weight, letterSpacing]) => {
            newUtilities[`.${category}-${weight}`].letterSpacing = letterSpacing;
          });
      });
